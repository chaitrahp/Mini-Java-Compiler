...
%token  IMPORT CLASS MAIN 
%token  STRING_LITERAL SYSTEM OUT PRINT
%left 	'+' '-'
%left 	'/' '*' '%'
%right 	'='
%right UMINUS
...
%token	<ival> 	INT CHAR	FLOAT 	VOID BOOLEAN
%token 	FOR  WHILE  IF ELSE
%token  PUBLIC PRIVATE PROTECTED STATIC STRING ARRAY_LIST
...
program
	...
	;
main
	: PUBLIC STATIC VOID MAIN '(' A ')' compound_statement
	;
A	:	STRING IDENTIFIER '['']'
	| 	STRING '['']' IDENTIFIER
	|	STRING '.''.''.' 
...
printstat
	...
	| SYSTEM '.' OUT '.' PRINT '(' STRING_LITERAL '+' expression ')'
	;
...
condition_statement
	...
	: IF '(' relational_expression ')' statement {create_node("if", 0);}  ELSE statement  {create_node("if", 0);}
	;
...
void printsymtable(){
    ...
    ftp=first;
    printf("\n\nSymbol Table\n\n");
	printf("   SYMBOL\t\t  NAME\t    TYPE\t      SCOPE\t      LINE \t  STORAGE(bytes)\t    	   VALUE \n"	 );
   	printf("\n");
    while(ftp!=NULL) {
        char data_type[10];
		int storage=0;
        ...
        printf("%11s\t%12s\t%8s\t\t%d\t\t%d\t\t%d\t\t\t",ftp->token, ftp->name, data_type, ftp->scope, ftp->lineno,storage);
